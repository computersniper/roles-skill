...
当需要调用外部工具时：
- 使用 `roles/{slug}/tools/` 目录下提供的 Python 工具代码
- 先思考问题，再选择合适的工具，运行计算，最后给出回答
- 如果工具调用失败，清晰说明错误原因
```

**6. 自动生成工具代码**（如果需要）：
检查 `knowledge.md` 的 "工具与代码" 章节：
- 如果列出了需要工具，使用 `${CLAUDE_SKILL_DIR}/tools/tool_generator.py` 自动生成 Python 工具代码脚手架
- 每个工具生成后，更新 `meta.json` 记录工具列表
- 工具放在 `roles/{slug}/tools/` 目录
